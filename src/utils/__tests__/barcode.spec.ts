...
describe("parseAIDataMatrix", () => {
  ...
  it("should parse format with Group Separators (GS): <GS>01...10...<GS>17...21...", () => {
    ...
    const barcode =
      "\u001d010880999999999710TK1234\u001d1725111121TK1234000093";
    const result = parseAIDataMatrix(barcode);
    expect(result).toEqual({
      ic_code: "08809999999997",
      serial_number: "TK1234000093",
  ...
  it("should be pass", () => {
    const barcode = "01088518814162281094160011729010621946001000047"
    const result = parseAIDataMatrix(barcode);
    expect(result).toEqual({
      ic_code: "08851881416228",
      serial_number: "946001000047",
    });
  })
});
